...
//            testcases = in.nextInt();
            ...
            while(testcases-- > 0) {
                ...
                /*
                    ...
                    They have mentioned a condition i.e. we have to include the elements that follow conditions!

                    i.e. ai-bj == i-j
                    This can be arranged as ai-i == bj-j;

                    So, by having a map that counts every ai-i And then adding those ai's into the key!

                    At last calculating the overall sum of values of the key's separate!
                ...
                for(int i = 1; i <= siz; i++) {
                    ...
                    if(!map.containsKey((nums[i]-i))) {
                        map.put((nums[i]-i),new ArrayList<>()); // kepping ai-i into the map
                        map.get((nums[i]-i)).add(nums[i]);
                    } else {
                        map.get((nums[i]-i)).add(nums[i]); // keeping the a[i] if key is already present!
                    }
                }
                ...
                for(int key : map.keySet()) {
                    ans = max(ans,sum(map.get(key)));
                }
                out.println(ans);

                // You can also add them without maintaining a list!

...
//            Comparator<Pair> com = Comparator.comparingInt(i -> i.v1);
            ...
            while(testcases-- > 0) {
                ...
                /*
                    Credits : https://codeforces.com/contest/1374/submission/85315014
                 */
                ...
                for(int i : nums) {
                    long toAdd = (k-(i%k))%k; // checking what number to be added to get divided by the number k
                    if(toAdd == 0) continue; // if it's divisible no use
                    int times = timesUsed.getOrDefault(toAdd,0); // else we have to know how many times it's repeated!
                    timesUsed.put(toAdd,times+1); // update its frequency by 1
                    toAdd += k*(long)times; // then you have to go at least go that value of x in-order to make that number divisible by x
                    ans = max(ans,toAdd+1); // then updating the answer!
                }

...
//            Comparator<Pair> com = Comparator.comparingInt(i -> i.v1);
            ...
            while(testcases-- > 0) {
                ...
                a[0] = m; // putting the element m in the 0th index of array A
                ...
                Arrays.sort(a); // sorting them

                Arrays.sort(b); // sorting.

                // sorting is done in order to remove the numbers efficiently from both the array's

                int l = 0; // pointing to the 0 th index of array b

                int r = siz-1; // pointing to the end of a array

                int start = 0; // pointing to the start of the array

                int deletions = 0; // counting the number of deletions!

                while(start <= r && l < siz) { // the start <= r because if it's greater then it states that the values which are being taken are being deleted!

                    if(a[start] >= b[l]) { // in this we will try to remove the min elements in B and max elements in A
                        l++; // deleting small element in B
                        r--; // deleting the larger element in A
                        deletions++; // so the operation is done!
                    } else {
                        l++; // else the pair is valid no need to delete that!
                        start++;
                    }
                }

                out.println(deletions); // at last print the number of elements that are deleted!

...
//            Comparator<Pair> com = Comparator.comparingInt(i -> i.v1);
            ...
            while(testcases-- > 0) {
                ...
                /*
                    ...
                    Question stated that we have to find the maximum size of the gift!

                    And in the gift the same number of chocolates shouldn't be present of any two type's of chocolates

                    we are given the chocolates as the input
                ...
                int[] freq = new int[siz+1]; // counting the frequency of each type of chocolate
                ...
                Arrays.sort(freq); // arranging them in a sorted order in-order to take maximize the gift

                int toAdd = Integer.MAX_VALUE; // initially nothing is added

                long sum = 0;

                for(int i = siz; i >= 1; i--) {
                    toAdd = Math.min(freq[i],toAdd-1); // considering the minimum of currentPick and the freq
                    if(toAdd <= 0) break;
                    sum += toAdd; // adding it to the sum!
//                    out.println(toAdd);
                }

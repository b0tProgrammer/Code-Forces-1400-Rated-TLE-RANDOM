...
//            testcases = in.nextInt();
            ...
            while(testcases-- > 0) {
                ...
                /*
                    Credits : https://codeforces.com/contest/1195/submission/57337392
                 */

                int siz = in.nextInt();

                long[][] grid = new long[2][siz]; // taking input!
                long[][] dp = new long[2][siz];

                for(int i = 0; i < siz; i++) {
                    grid[0][i] = in.nextLong();
                    dp[0][i] = -1;
                }

                for(int i = 0; i < siz; i++) {
                    grid[1][i] = in.nextLong();
                    dp[1][i] = -1;
                }

                out.println(max(ans(siz-1,0,grid,dp),ans(siz-1,1,grid,dp)));
            }
            out.close();
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private static long ans(int idx,int last, long[][] grid,long[][] dp) {
        if(idx < 0) return 0;
        ...
        if(last == 0) {
            ans = max(grid[0][idx]+ans(idx-1,1,grid,dp),ans(idx-1,0,grid,dp)); // pick the current thing or skip
        } else {
            ans = max(grid[1][idx]+ans(idx-1,0,grid,dp),ans(idx-1,1,grid,dp));
        }

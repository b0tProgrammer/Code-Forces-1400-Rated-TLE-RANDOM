...
//            Comparator<Pair> com = Comparator.comparingInt(i -> i.v1);
            ...
            while(testcases-- > 0) {
                ...
                /*
                    Credits : https://www.youtube.com/watch?v=Z9jEFjHFE0o
                 */

                int siz = in.nextInt(); // using DP!
                int[] nums = new int[siz+1];
                int[] dp = new int[siz+1];
                for(int i = 1; i <= siz; i++) {
                    nums[i] = in.nextInt();
                    dp[i] = -1;
                }
                int ans = 1; // The min answer is element itSelf
                for(int i = 1; i <= siz; i++) {
                    ans = max(ans,1+fun(nums,i,dp)); // considering the element itself and the next multiple of it!
                }
    ...
    private static int fun(int[] nums, int v, int[] dp) {

        if(v >= nums.length) return 0; // checking condition!

        int c = 0; // initially no elements discovered

        if(dp[v] != -1) return dp[v]; // If that element is already present or counted we return that

        for(int i = 2*v; i < nums.length; i += v) {
            if(nums[i] > nums[v]) {
                c = max(c,1+fun(nums,i,dp)); // else we will do the same stuff as mentioned!
            }
        }

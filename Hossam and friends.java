...
//            Comparator<Pair> com = Comparator.comparingInt(i -> i.v1);
            ...
            while(testcases-- > 0) {
                // we are given that we have find the maximum number of pairs such that in that valid range the members should be be known to each other
                int siz = in.nextInt();
                int pairs = in.nextInt();

                int[] maxPos = new int[siz+1]; // finding up to what max valid position can we go!

                Arrays.fill(maxPos,siz+1); // thinking that we are having all positions valid!
                ...
                for(int i = 1; i <= pairs; i++) {
                    ...
                    if(start > end) { // if the start position is greater, then we have to update it!
                        int t = start;
                        start = end;
                        end = t;
                    }

                    maxPos[start] = min(maxPos[start],end); // the max valid position till she can go!

                }

                for(int i = siz-1; i >= 1; i--) {
                    maxPos[i] = min(maxPos[i],maxPos[i+1]); // as we know that in any range each should be known to each other so,
                    // We have to update the maximums from the back also!because if there is no range, for that then it's okay to take all of them.
                }

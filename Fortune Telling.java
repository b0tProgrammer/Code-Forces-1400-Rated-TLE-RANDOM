...
//            Comparator<Pair> com = Comparator.comparingInt(i -> i.v1);
            ...
            while(testcases-- > 0) {
                /*
                    Fortune Telling
                    In this, we are given 3 integers n,x,y
                    n is the size of the array , x is the start value of alice and y is the final required value
                    bob start's with x+3.
                    let's say Alice's value as A and Bob's value as B
                    There are n elements as input,
                    and we can perform 2 operations :
                    either A+(current index element) or A^(current index element);
                    so, bob can also any of operations mentioned above
                    Now, we have to say after performing those operations, whether they can achieve the value y or not!
                 */

                /*
                    There is a property common for both + and ^ (i. e the parity)
                    If you add any two numbers then the result is even
                    similarly if you xor any two elements then the value is even!
                    So, we can say that whether the parity of the A and B are same as y after performing operations!
                    If they are having same parity then we can say that we can achieve that in any manner!
                ...
                long y = in.nextLong();
                for(int i = 1; i <= siz; i++) {
                    x ^= in.nextInt(); // the parity doesn't change
                }
                if(odd(x) == odd(y)) out.println("Alice"); // checking for same parity
